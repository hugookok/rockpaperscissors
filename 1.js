...
function startround() {
  ...
  setTimeout(function () {
    replacetimer(resetquestionmark());
  }, 2000);
  setTimeout(function () {
    replacetimer("2");
  }, 2000);
  ...
  //Bytter til valgte våpen(stein saks eller papir) og bilde
...
//Bytter begge våpnene til spørsmålstegn på slutten av runden
function resetquestionmark() {
  resultimg1.src = "img/questionmark.png";
  if (humanselected == "") {
    playershower.src = "img/questionmark.png";
  }
  document.getElementById("winnerthing").className = "normaltext"; 
  document.getElementById("winnerthing").innerHTML = ""; 
...
function timerdone() {
  if (humanselected == "") {
    ...
    //bytte bilde på spørsmålstegn for robot
    if (robotchoice == 1) {
...
//regner poeng og resultatmelding, starter ny runde eller resetter lukker spillet
function result(message, winlossordraw) {
  document.getElementById("winnerthing").innerHTML = message;
  roundcounter += 1;
  if (winlossordraw == "loss") {
    points -= 1;
    document.getElementById("winnerthing").className = "redtext"; 
  }
  if (winlossordraw == "won") {
    points += 1;
    document.getElementById("winnerthing").className = "greentext"; 
  }
...
//sluttresultat, spiller lyd og bytter bilde
function finished() {
